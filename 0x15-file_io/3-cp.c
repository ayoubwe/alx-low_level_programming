...
/**
 * check97 - checks for the correct nmb
 * @argc: int
 * Return: Nothing
 */
...
void check98(ssize_t check, char *file, int fd_from, int fd_to)
{
	if (check == -1)
	{
		dprintf(STDERR_FILENO, "Error:You can't read from file %s\n", file);
		if (fd_from != -1)
			close(fd_from);
		if (fd_to != -1)
			close(fd_to);
...
/**
 * check100 - checks that file
 * @check: int
 * @fd: int
 * Return: Nothing
 */
void check100(int check, int z)
{
	if (check == -1)
	{
